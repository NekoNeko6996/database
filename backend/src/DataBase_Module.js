...
async function Read(Client, Collection, Condition, Limit) {
  ...
  const cursor = await Client.collection(Collection)
    .find(Condition)
    .sort({ date: -1 })
    .limit(Limit || 100);
...
async function Delete(Client, Collection, id) {
  await Client.collection(Collection).deleteOne(
    { _id: new ObjectId(id) },
    (err, result) => {
      if (err) throw err;
      console.log(result);
  ...
  try {
    ...
    switch (type) {
      ...
      case "fullLoad":
        ...
        const result_fullLoadSpend = await Read(
          DataBaseClient,
          "Spend_Data",
          {}
        );
        const result_fullLoadReceive = await Read(
          DataBaseClient,
          "Receive_Data",
          {}
        );
        ...
        const result_SpendSince = await Read(DataBaseClient, "Spend_Data", {
          date: { $gte: "2023-09-23" },
        });
        ...
        return {
          ...
          AccountBalance:
            (await Math_Module.SumDataAmount(result_fullLoadReceive)) -
            (await Math_Module.SumDataAmount(result_SpendSince)),
        };
      case "MonthlyDataAggregation":
        ...
        const result_Month_after_calculate = await Math_Module.SumDataAmount(
          result_Month_raw
        );

        const Month_Find = await Read(DataBaseClient, "Data_Month", {
          month: TimeFilter.since.slice(5, 7),
        });
      ...
      case "searchRequest":
        ...
        //trả về//
        return await Read(DataBaseClient, "Spend_Data", Document);
