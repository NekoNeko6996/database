...
app.post("/onloadRequest", async (req, resp) => {
  try {
    ...
    const result = await DataBase_Module.DataBase(
      "",
      "reload",
      req.body.TimeFilter
    );

    await DataBase_Module.DataBase(
      "",
      "MonthlyDataAggregation",
      req.body.ConstTimeFilter
    );
...
app.post("/SearchRequest", async (req, resp) => {
  try {
    console.log(req.body);
    const ObjectResult = await Math_Module.ConversionStringToObject(
    ...
    const SearchResult = await DataBase_Module.DataBase(
      ObjectResult,
      "searchRequest"
    );
