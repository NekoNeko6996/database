...
function App() {
  ...
  const SearchRequest = async (event) => {
    ...
    result = await result.json();
    if (!result) {
      swal("Error", "Please Check Syntax", "warning");
    }
    setDataResp(result);
  };
  ...
  return (
    <>
      <div
        className="TopBar-box-div"
        style={{ height: TopBarHeight }}
        onClick={(event) => topBarClick(event.target.className)}
      >
        <button
          className="leftBoxHide-button"
          onClick={(event) => onButtonHideClick(event.target)}
        >
          <span id="top"></span>
          <span id="bottom"></span>
        </button>
        ...
        <div>
          <select
            name="tag"
            className="Select-tag-sel"
            onChange={(event) => setPageTag(event.target.value)}
          >
        ...
        <div className="Since-ToDateBox-div">
          ...
          <input
            ...
            value={Since}
            onChange={(event) =>
              setSince(JSON.stringify(event.target.value).slice(1, 11))
            }
            className="dateSince-input"
          />
          <input
            type="date"
            value={ToDate}
            onChange={(event) =>
              setToDate(JSON.stringify(event.target.value).slice(1, 11))
            }
            className="dateToDate-input"
          />
          <div id="reload-box-btn">
            <button
              className="button-rainbow"
              id="reload"
              onClick={DataBaseLoad}
            ></button>
          </div>
        </div>
      </div>
      ...
      <div id="bodyContainer">
        ...
        <div className="LeftBox-div" id="LeftBox-close">
          <form action="">
            ...
            <input
              type="text"
              placeholder="Name giver..."
              value={NameGiver}
              onChange={(event) => setNameGiver(event.target.value)}
              className="InputNameGiver-input"
            />
            <input
              type="number"
              ...
              className="InputReceive-input"
            />
            <input
              type="submit"
              className="ReceiveSubmit-button"
          ...
          <form action="" className="Form-box-form-left">
            ...
            <input
              ...
              className="trading-name-input-left"
            />
            <input
              type="number"
              placeholder="Amount..."
              ...
              className="spend-amount-input-left"
            />
            <select
              name="tag-select"
              className="currency-select-left"
              value={Currency}
            ...
            <input
              ...
              className="spend-date-input-left"
            />

            <input
              type="text"
              placeholder="tag..."
              className="spend-tag-select-left"
              list="spend-tag-list-id"
              value={SpendTag}
              onChange={(event) => setSpendTag(event.target.value)}
            />
          ...
          <div className="user-box">
            <img
              ...
              src="https://i.pinimg.com/originals/86/dd/bb/86ddbb9654b59b76257867031c864407.jpg"
              alt="load err"
            />
        ...
        <div className="Right-Box-div">
          <form action="" className="Form-box-form">
            ...
            <input
              ...
              value={DatePurchase}
              onChange={(event) => setDatePurchase(event.target.value)}
              className="spend-date-input"
            />

            <input
              type="text"
              placeholder="tag..."
              className="spend-tag-select"
              list="spend-tag-list-id"
              value={SpendTag}
              onChange={(event) => setSpendTag(event.target.value)}
            />
